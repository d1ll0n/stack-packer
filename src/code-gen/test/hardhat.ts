...
import { AbiElementaryType, ArrayJoinInput, CodeGenFunction, ProcessedField, ProcessedStruct } from "../../types";
import { shouldCheckForOverflow } from "../fields";
import { TestValueKind, GetterFn, SetterFn, ElementaryValuesGetter, TestEnvironment, GetterOrSetter } from './generic'
...
type SetterFunctionTest = {

}

class HardhatTestEnvironment extends TestEnvironment {
  constructor() {
    super(
      HardhatElementaryValues,
    );
  }
...
const _env = new HardhatTestEnvironment()
const getFieldValue = (field: ProcessedField, kind: TestValueKind) => _env.getFieldValue(field, kind);
...
const _generateCall = (contractName: string, fn: {name:string}, args: string[]) => `${contractName}.${fn.name}(${args.join(", ")})`;
const generateIt = (name: string, code: ArrayJoinInput<string>[]) => [
  `it('${name}', async () => {`,
  code,
  '});'
]
const _getOverflowTest = (contractName: string, fn: {name:string}, args: string[], field: ProcessedField) => generateIt(
  `Reverts when ${field.name} overflows`,
  [
    `await expect(`,
...
const _getUnderflowTest = (contractName: string, fn: {name:string}, args: string[], field: ProcessedField) => generateIt(
  `Reverts when ${field.name} underflows`,
  [
    `await expect(`,
    _generateCall(contractName, fn, args),
    ').to.be.revertedWith("0x11 (Arithmetic operation underflowed or overflowed outside of an unchecked block)");'
...
export function generateHardhatTestForFunction(
  ...
  const generateCall = (args: string[]) => _generateCall(contractName, fn, args)
  const getOverflowTest = (args: string[], field: ProcessedField) => _getOverflowTest(contractName, fn, args, field)
  const getUnderflowTest = (args: string[], field: ProcessedField) => _getUnderflowTest(contractName, fn, args, field)
  
  const fields = fn.internalType === 'getter' ? fn.outputFields : fn.inputFields;
  ...
  if (fn.internalType === 'setter') {
    ...
    const testSetMinMax = getSetterTest(
      `Should be able to set ${fields.length === 1 ? 'min value' : 'min/max values'}`,
      fields,
      [generateCall(minMax)],
      getterCode,
      minMax
    ...
    const testSetMaxMin = getSetterTest(
      `Should be able to set ${fields.length === 1 ? 'max value' : 'max/min values'}`,
      fields,
      [generateCall(maxMin)],
      getterCode,
      maxMin
    ...
    const getterCode = [`const ${destructuredDecodeParams} = await ${contractName}.${fn.name}()`]

    const getSetter = _env.findGetterOrSetterForFields(struct, fields, fn, contractName, 'setter');
    if (!getSetter) {
    ...
    const testGetMinMax = getSetterTest(
      `Should be able to get ${fields.length === 1 ? 'min value' : 'min/max values'}`,
      fields,
    ...
    const testGetMaxMin = getSetterTest(
      `Should be able to get ${fields.length === 1 ? 'max value' : 'max/min values'}`,
      fields,
...
export function generateHardhatTest(
  struct: ProcessedStruct,
  ...
  for (const fn of functions) {
    const code = generateHardhatTestForFunction(struct, fn, libraryName)
    if (code) fnTests.push(...code, '');
  }
  removeTrailingNewLines(fnTests)
  // const fnTests = functions.map(fn => generateHardhatTestForFunction(fn, libraryName)).filter(Boolean);
  const contractName = `external${libraryName}`;
  const testFile = [
