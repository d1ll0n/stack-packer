...
function generateExternalFunction(
  fn: CodeGenFunction,
  ...
  if (fn.internalType === "setter") {
    copyForExternalFunction.stateMutability = undefined;
  } else if (fn.internalType === "getter") {
    copyForExternalFunction.stateMutability = "view";
  } else {
    return undefined;
  }
  ...
  if (hasStructOutput) {
    ...
    copyForLibraryCall.outputs[0].definition = definition.replace(
      name,
      structStorageName
    );
  ...
  copyForExternalFunction.body = generateInternalLibraryCall(
    copyForLibraryCall,
    libraryName
  );
...
export function generateExternalCoder(
  ...
  for (const fn of functions) {
    const externalFn = generateExternalFunction(
      fn,
      structStorageName,
      libraryName
    );
  ...
  const externalCode = [
    ...
    [`${structTypeName} internal ${structStorageName};`, ...code],
    "}",
  ];
  return {
    externalCode,
