...
export type SetterFn = (
  values: string[],
  defaultValueKind: TestValueKind
) => ArrayJoinInput<string>;
...
export class TestEnvironment {
  ...
  getFieldValue = (field: ProcessedField, kind: TestValueKind) => {
    if (field.type.meta === "elementary") {
      ...
      return this.elementaryValuesGetter[field.type.type][kind](
        field.type.size
      );
    }
  ...
  findGetterOrSetterForFields(
    ...
    const group = struct.groups.find(
      (g) =>
        `${type === "getter" ? "get" : "set"}${g.name.toPascalCase()}` ===
        fn.name
    );
    ...
    const usableGroup = group
      ? group[generateParamName] && group
      : struct.groups.find((g) => {
          const groupFields = g.fields.map((f) => f.name);
          return (
            g[generateParamName] &&
            !fields.some((f) => !groupFields.includes(f.name))
          );
        });
      ...
      if (type === "getter") {
        return () => {
          return fields.map((field, i) =>
            _getCallWithAssignment(field.getterName, [], [field.name])
          );
        ...
        return (values: string[], defaultValueKind: TestValueKind) => {
          return fields.map((field, i) =>
            _getCallWithAssignment(field.setterName, [values[i]], [])
          );
    ...
    if (type === "getter") {
      return () => {
        return _getCallWithAssignment(
          refObj.getterName,
          [],
          fields.map((f) => f.name),
          refObj.fields.length > 1
        );
      ...
      return (values: string[], defaultValueKind: TestValueKind) => {
        ...
        const allValues = refObj.fields.map((field) => {
          return (
            fieldsToValues[field.name] ||
            this.getFieldValue(field, defaultValueKind)
          );
        });
