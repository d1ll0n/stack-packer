...
function getA(Data1 encoded) internal pure returns (uint256 a) {
	assembly {
...
function getB(Data1 encoded) internal pure returns (int256 b) {
	assembly {
		b := signextend(0x01, shr(Data1_b_bitsAfter, encoded))
	}
}
...
function getC(Data1 encoded) internal pure returns (int256 c) {
	assembly {
		c := signextend(0x03, shr(Data1_c_bitsAfter, encoded))
	}
}
...
function getD(Data1 encoded) internal pure returns (bool d) {
	assembly {
		d := and(MaxUint1, shr(Data1_d_bitsAfter, encoded))
	}
}
